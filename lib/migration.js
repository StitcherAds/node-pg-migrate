...
module.exports = class Migration {
  ...
  _getMarkAsRun(action) {
    ...
    const { migrationsTable, isRedshift } = this.options;
    const { name } = this;
    const nowFunction = isRedshift ? "getdate()" : "NOW()";
    switch (action) {
      case this.down:
      ...
      case this.up:
        ...
        return `INSERT INTO "${schema}"."${migrationsTable}" (name, run_on) VALUES ('${name}', ${nowFunction});`;
      default:
