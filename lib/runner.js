...
const ensureMigrationsTable = async (db, options) => {
  try {
    ...
    const { migrationsTable, isRedshift } = options;
    const fullTableName = {
      schema,
    ...
    if (migrationTables && migrationTables.length === 1) {
      ...
      const idColumnType = isRedshift ? "bigint identity(1,1)" : "SERIAL";
      await db.query(
        template`CREATE TABLE "${fullTableName}" ( ${idColumn} ${idColumnType} PRIMARY KEY, ${nameColumn} varchar(255) NOT NULL, ${runOnColumn} timestamp NOT NULL)`
      );
    }
  } catch (err) {
...
const runner = async options => {
  ...
  const { isRedshift, noLock } = options;
  ...
  try {
    if (options.schema) {
      ...
      const setSchema = isRedshift ? "SET search_path to" : "SET SCHEMA";
      await db.query(`${setSchema} '${options.schema}'`);
    }
